...
# 1 "mcc_generated_files/../MultiServo.h" 1
# 15 "mcc_generated_files/../MultiServo.h"
    char SERVO3,SERVO4,SERVO5,SERVO6,SERVO7 ,SERVO8;

    const uint16_t Ticks4Window = 0x9C40;
    const uint16_t Ticks4Minimum = 0X2580;
    const uint16_t Ticks4Center = 24000;
    const uint16_t Ticks4Maximum = 0x8fc0;

    uint8_t command;
    uint16_t Servo_PWM[8]={0x8fc0,0X8FC0,0,0,0,0,0,0};
    uint8_t Servo_Idx=0;
    _Bool SERVO1_ON=1;
    _Bool SERVO2_ON=1;
    _Bool SERVO3_ON=0;
...
void __attribute__((picinterrupt(("")))) INTERRUPT_InterruptManager (void)
{
    ...
    if(INTCONbits.PEIE == 1)
    {
        ...
        else if(PIE4bits.TMR5IE == 1 && PIR4bits.TMR5IF == 1)
        {
            ...
            if(flag_Phase==0){
                if(Servo_Idx==0 && SERVO1_ON) LATBbits.LATB0 = 1;
                if(Servo_Idx==1 && SERVO2_ON) LATBbits.LATB1 = 1;
                ...
                TMR5_WriteTimer(Ticks4NextInterrupt);
            }
            if(flag_Phase==1){
                if(Servo_Idx==0 && SERVO1_ON) LATBbits.LATB0 = 0;
                if(Servo_Idx==1 && SERVO2_ON) LATBbits.LATB1 = 0;
