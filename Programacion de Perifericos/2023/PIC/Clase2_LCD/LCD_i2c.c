...
void lcd_setCustomChar(unsigned char dir, unsigned char *dato)
{
   //Donde dir es la posicion en donde se escribiran los caracteres,va de 0 a 7
...
void lcd_showCustomChar( unsigned char dir)
{
   //CGRAM(dir);//Muestra el caracter especial almacenado en la posicion dir;*/
   lcd_send_byte(1,dir);
}
void lcd_showCustomCharXY(unsigned char x, unsigned char y, unsigned char dir)
{
   lcd_gotoxy(x,y);
   //CGRAM(dir);//Muestra el caracter especial almacenado en la posicion dir;*/
   lcd_send_byte(1,dir);
