...
# 76 "mcc_generated_files/eusart.c" 2

volatile char caracter_rx;
...
void EUSART_Receive_ISR(void)
{
    ...
    caracter_rx = EUSART_Read();
    if(caracter_rx == '*'){
        buffer_rx[contador_rx] = '\0';
        contador_rx = 0;

        if(strstr(buffer_rx,"encendido")){
            do { LATAbits.LATA2 = ~LATAbits.LATA2; } while(0);
        }
        if(strstr(buffer_rx,"apagado")){
            do { LATAbits.LATA3 = ~LATAbits.LATA3; } while(0);
        }
    }else{
        buffer_rx[contador_rx++] = caracter_rx;
    }
