...
void lcd_setCustomChar(uint8_t *dato, uint8_t dir);
...
void Write_PCF(char dato){
        ...
        I2C1_Write1ByteRegister( 32|(7),0, (dato | (1<<3)) );
# 24 "LCD_i2c.c"
}
...
void Send_D7_D4(uint8_t address,uint8_t nibble){
   ...
   if((nibble & 1)==1) datos= datos|(1<<4); else datos = datos&~(1<<4);
   if((nibble & 2)==2) datos= datos|(1<<5); else datos = datos&~(1<<5);
   if((nibble & 4)==4) datos= datos|(1<<6); else datos = datos&~(1<<6);
   if((nibble & 8)==8) datos= datos|(1<<7); else datos = datos&~(1<<7);
